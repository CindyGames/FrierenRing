...
public class OptimizedSpawnerReverse : MonoBehaviour
{
    [System.Serializable]
    ...
    public float despawnDistance = 10f;         // Дистанция для удаления объектов над экраном
    ...
    void Update()
    {
        if (spawnBack)
        {
            // Проверяем, если игрок прошел на spawnDistance вниз, то создаем новые объекты
            if (lastSpawnY - playerTransform.position.y > spawnDistance)
            {
                SpawnNextObject();
            }
    ...
    void SpawnObjectAtOffset(float yOffset)
    {
        ...
        // Устанавливаем смещение Y для спавна ниже экрана
    ...
    // Метод для удаления объектов, которые находятся над экраном
    void DespawnOldObjects()
    {
        ...
        {
            // Проверяем, существует ли объект и находится ли он над экраном
